...
WHERE datetime >= '".$from."' 
AND datetime <= '".$to."' and trnstype='desposite' and status='Success' order by pid DESC"); 
...
AND datetime <= '".$to."' and trnstype='desposite' and status='Success' order by pid DESC");
	   ...
	   $toutotal=mysqli_query($conn, "select sum(amount) as total from payment where trnstype='desposite' and status='Success' order by pid DESC");
	    $touserinfo=mysqli_query($conn, "select * from payment where trnstype='desposite'  order by pid DESC");
   }
   ...
   ?>
   <h2> Total Deposite : <?php echo $ttotal['total'];?></h2>
