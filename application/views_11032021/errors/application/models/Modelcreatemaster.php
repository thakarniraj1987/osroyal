...
class Modelcreatemaster extends CI_Model
{
	...
	function saveCreateMaster(){
				...
				if($chek==0){
			        ...
			        $wortype="ADD_ACC";
			        $remarks=$this->input->post('typeId').">>".$this->input->post('username').">>".$this->input->post('partner');
			        ...
			        if ($this->input->post('typeId')==1) {
			        	...
			        	$adminPval=100-$this->input->post('partner');
			        	$masterPval=$this->input->post('partner');
			        	$dealerPval=0;

			        }else if($this->input->post('typeId')==2){

			        	$adminPval=100-$this->input->post('PntPartenerShip');
			        	$masterPval=$this->input->post('PntPartenerShip')-$this->input->post('partner');
			        	$dealerPval=$this->input->post('partner');
	...
	function saveFancy(){
			if ($_POST['fancyType']==1) {
					$insertData = array('HeadName'=> $_POST['HeadName'],'TypeID'=> $_POST['fancyType'],'MatchID'=> $_POST['mid'],'Remarks'=>$_POST['remarks'],'active'=>1,'date'=>date('Y-m-d'),'time'=>date('H:i:s'));
					$this->db->trans_begin();
				...
				/*$num=$this->chkMatchFancy($_POST['mid'],$_POST['fancyType']);
				if($num==0){*/
					$insertData = array('HeadName'=> $_POST['HeadName'],'TypeID'=> $_POST['fancyType'],'MatchID'=> $_POST['mid'],'Remarks'=>$_POST['remarks'],'date'=>$_POST['date'],'time'=>$_POST['time'],'fancyRange'=>$_POST['fancyRange'],'active'=>1);
					//$query=$this->db->insert('matchfancy', $insertData);
						/*start*/
					$this->db->trans_begin();
					/*Get Match Name*/
				...
				/*}else{
					...
					$insertData = array('HeadName'=> $_POST['HeadName'],'TypeID'=> $_POST['fancyType'],'MatchID'=> $_POST['mid'],'Remarks'=>$_POST['remarks'],'date'=>$_POST['date'],'time'=>$_POST['time'],'active'=>1,'upDwnLimit'=>$_POST['liability']);
					...
					$insertData = array('HeadName'=> $_POST['HeadName'],'TypeID'=> $_POST['fancyType'],'MatchID'=> $_POST['mid'],'date'=>$_POST['date'],'time'=>$_POST['time'],'HeadName'=>$_POST['HeadName'],'rateDiff'=>$_POST['rateDiff'],'pointDiff'=>$_POST['pointDiff'],'MaxStake'=>$_POST['MaxStake'],'Remarks'=>$_POST['remarks'],'active'=>0,'upDwnLimit'=>$_POST['liability']);
			...
			if ($_POST['userType_id']==$_POST['SltUsrType_id']) {
				...
				if ($getPassword==sha1($_POST['oldPassword'])) {
					...
					//start user working table save the data By Manish 31/12/2016
			        	...
			        	$remarks="USER TYPE_ID : ".$_POST['userType_id'].">>USER_ID : ".$_POST['userId'].">>";
			        	$userWrkingArray = array(
					...
					//start user working table save the data By Manish 31/12/2016
			        	$wortype="CHANGE_PASSWORD";
			        	$remarks="USER TYPE_ID : ".$_POST['userType_id'].">>USER_ID : ".$_POST['userName'].">>";
			        	$userWrkingArray = 	array(
									            'woruser' 			=> $_POST['userId'],
									            'wormode' 			=> 0,
									            'wordate' 			=> date('Y-m-d H:i:s',now()),
		...
		function lockUser(){
			...
			//start user working table save the data By Manish 31/12/2016
        	$wortype="UserLock";
        	$remarks="USER TYPE_ID : ".$_POST['userType'].">> USER_ID : ".$_POST['userName'];
        	$userWrkingArray = 	array(
						            'woruser' 			=> $_POST['userId'],
						            'wormode' 			=> 1,
						            'wordate' 			=> date('Y-m-d H:i:s',now()),
           ...
           //echo "call sp_updLockStatus($userId,$userType,$lockVal,$lockVal,$loginUserID)";die();
            $query =$this->db->query("call sp_updLockStatus($userId,$userType,$lockVal,$lockVal,$loginUserID)");
			...
			//start user working table save the data By Manish 31/12/2016
	        	...
	        	$userWrkingArray = 	array(
	 			...
	 			$this->db->trans_begin();
					...
					$dataArray = array('lgnusrlckbtng' => $_POST['lockbettingVal']);
					$this->db->where('mstrid',$_POST['userId']);
		...
		function UpdateUserAccount(){
			...
			//start user working table save the data By Manish 31/12/2016
				...
				//End of useworking table
						...
						$dataArray = array('lgnusrCloseAc' => $_POST['accValue']);
						$this->db->where('mstrid',$_POST['userId']);
						$q1=$this->db->update('createmaster', $dataArray);
		...
		{
			//[userId] => Master5 [Name] => Master5 [partnership] => 60.00 [Commission] => 20.00 [maxProfit] => 10000 [maxLoss] => 10000 [maxStake] => 0 [userType] => 1 [PntPartenerShip] => 100.00 [parantId] => 1
			$dataArray = array(
								'mstrname' => $_POST['Name'],
	...
	{
		$this->db->select('mstrlock,lgnusrlckbtng,lgnusrCloseAc,IFNULL(lgnUserMaxStake,"0") as stakeLimit,lgnUserMaxProfit,lgnUserMaxLoss');
		$this->db->from('createmaster');
	...
	function suspendFancy(){
		...
		$row = $this->db->query('SELECT MAX(MFancyID) AS `maxid` FROM `matchfancy`')->row();						
		$maxid = $row->maxid+1; 
		$dataArray = array( 'active' => $_POST['status'],'SessInptYes'=>$_POST['YesVal'],'SessInptNo'=>$_POST['NoVal'],'MaxStake'=>$_POST['MaxStake'],'NoValume'=>$_POST['NoValume'],'YesValume'=>$_POST['YesValume'],'pointDiff'=>$_POST['pointDiff'],'MFancyID'=>$maxid);
	...
	function updateUserAcPasswrd($userId,$passwrd){
			$dataArray = array('mstrpassword' => sha1($passwrd));
			$this->db->where('mstrid',$userId);
	...
	function submitClearChip()//sourabh 161229
	{
		...
		$query = $this->db->query("call sp_insClearChip($UserID,$LoginID,$CrDr,$Chips,$IsFree,'$Narration',$ParantId)");
		$res = $query->result_array();
			$query->next_result();
			$query->free_result();
			return $res;
	...
	function editFancy(){//sourabh 170118
			...
			if(empty($_POST['fancyType'])==1)  $pTypeID=0;else $pTypeID=$_POST['fancyType'];
			...
			if(empty($_POST['FancyId'])==1) $pID=0;else $pID=$_POST['FancyId'];
    ...
    function updatePartnerShip($admin,$master,$dealer,$userId){
    	$dataArray = array( 'Admin' => $admin,'Master'=>$master,'Dealer'=>$dealer);
        $this->db->where('UserID',$userId);
        $query=$this->db->update('tblpartner', $dataArray);
        //echo $this->db->queries[0];
        return $query;
    }
     function updateUserCommission($oddsComm,$sessionComm,$otherComm,$ID){
    	$dataArray = array( 'Commission' => $oddsComm,'SessionComm'=>$sessionComm,'OtherComm'=>$otherComm);
        $this->db->where('mstrid',$ID);
        $query=$this->db->update('createmaster', $dataArray);
        //echo $this->db->queries[0];die();
        return $query;
