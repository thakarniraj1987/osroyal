...
//		       $allowAuth = array('chkLoginUser','getMarketListing','matchLstIndianSessionPublic');
        ...
        //	$data = $this->Modeleventlst->mobileGetMrktByMatchId($matchId);
        	...
        	if(!empty($data)){
        		...
        		$dbMatch = array('matchName'=>$mdata['matchName'],'series_name'=>$mdata['series_name'],'matchid'=>$mdata['matchid'],'marketid'=>$mdata['marketid'],'market_name'=>$mdata['market_name'],'matchdate'=>$mdata['MstDate'],'sportname'=>$mdata['sportname'],'is_favourite'=>$mdata['is_favourite'],'SportId'=>$mdata['SportId'],'max_bet_liability'=>$mdata['max_bet_liability'],'max_market_liability'=>$mdata['max_market_liability']);
        	...
        	/*	$marketArr = explode(',', $mdata['marketid']);
                ...
                if($data[0]['isManualMatchOdds']==1){
                    $response["isManualMatchOdds"] = 1;
                    $manualMatchOddsDetailsTemp =[];
                    $runner = [];
                    $runnerArray = json_decode($data[0]['match_runner_json'],true);

                    $manualMatchOddsDetails = $this->Modelcreatemaster->manualMatchOddsDetails($data[0]['marketid']);
                    if(!empty($runnerArray)){
                        foreach ($runnerArray as $key=>$runnerRec){
                            $key = $key+1;
                            if($key <= 2){
                                $runner[]=['id'=>$runnerRec['id'],'name'=>$runnerRec['name'],'back'=>$manualMatchOddsDetails['team'.$key.'_back']-1,'lay'=>$manualMatchOddsDetails['team'.$key.'_lay']-1];
                            }else{
                                $runner[]=['id'=>$runnerRec['id'],'name'=>$runnerRec['name'],'back'=>$manualMatchOddsDetails['draw_back']-1,'lay'=>$manualMatchOddsDetails['draw_lay']-1];
                            }
                    ...
                    $manualMatchOddsDetailsTemp =  $response["data"][0];
                    $manualMatchOddsDetailsTemp['runners']= $runner;
                    $response["runners_manual"] = $manualMatchOddsDetailsTemp;
                }
        	}else{
