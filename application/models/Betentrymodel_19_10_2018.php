...
class Betentrymodel extends CI_Model
{
	...
	function mobileValidateSaveBet($data=NULL){
		...
		if($isMatched == 0 && $config_matched=='N'){
			$result = array('code' => 9 ,'error'=>true,'message' => 'Odds changed');
            return $result;
		}
...
/*
	...
	function myBetsFilters($data){
        ...
        /*	print_r($data); 
       	...
       	// echo "CALL `GetBetHistoryFilterPaging`($userId, '$from_date', '$to_date' , $start , $page_limit)";
        //	die;  
	...
	function myBetsFiltersCount($data){
			...
			$query =$this->db->query("CALL `GetBetHistoryFilterPagingCount`($userId, '$from_date', '$to_date')");
			$res = $query->result_array();
			$query->next_result();
			$query->free_result();
			return $res;
	...
	function AcStatementFilter($data){
			...
			$query =$this->db->query("CALL `sp_acStatement_filters`($userId, '$from_date', '$to_date' , $start , $page_limit)");
	...
	function AcStatementFilterCount($data){
			...
			$query =$this->db->query("CALL `sp_acStatement_filters_count`($userId, '$from_date', '$to_date')");
...
/*	echo 	"SELECT *,ROUND((@variable := @variable + `Chips`),2)  AS `Balance` from viewacstatement where user_id = $userId order by Sdate DESC LIMIT $start,$limit;";
    ...
    function undoBetByMatchId($matchId){
        ...
        $this->db->trans_strict(FALSE);
         ...
         $this->db->query("INSERT INTO tblbets (LogInId, UserId, ParantId, MatchId, MarketId, SelectionId, selectionName, MstDate, Odds, Stack, P_L, isBack, IsMatched, Result, ResultID, IP_ADDESSS, deviceInfo, SAdmin, Admin, Master, Dealer, Chips, IsInPlay, isApp)
SELECT LogInId, UserId, ParantId, MatchId, MarketId, SelectionId, selectionName, MstDate, Odds, Stack, P_L, isBack, IsMatched, Result, ResultID, IP_ADDESSS, deviceInfo, SAdmin, Admin, Master, Dealer, Chips, IsInPlay, isApp
