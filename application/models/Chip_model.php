...
class Chip_model extends CI_Model
{
	...
	function validateSaveChipAc($data){
		...
		$result = array();
      	...
      	if($CrDr==1){
      		if($userId!=1){
	        	...
	        	$parentData = $this->getLiability($ParantId);
	        	if(isset($parentData[0]['Balance']) && $parentData[0]['Balance'] < $Chips){
	        		$result = array('error' => 1 ,'message' => 'Insufficient balance parent a/c');
	                return $result;
	        	}
      		}
		}elseif($CrDr==2){
        	$userData = $this->getLiability($userId);
        	if(isset($userData[0]['Balance']) && $userData[0]['Balance'] < $Chips){
        		$result = array('error' => 1 ,'message' => 'Insufficient balance child a/c');
                return $result;
        	}
		} 
