...
class Modelcreatemaster extends CI_Model
{
	...
	function getUserBalance($userId){
		$this->db->select('liability as Liability,balance as Balance,p_l as P_L,freechips as FreeChip,chip as Chip,sessionLiability as sessionLiability,unmatchliability as unmatchliability');
		$this->db->from('createmaster');
		$this->db->where('mstrid',$userId);		
		$query = $this->db->get();
	...
	function updateUserBalLiablity($userId){
		...
		$dataArray = array('liability' => $data[0]['Liability'],'balance' => $data[0]['Balance'],'p_l'=>$data[0]['P_L'],'freechips'=>$data[0]['FreeChip'],'chip'=>$data[0]['Chip'],'sessionLiability'=>$data[0]['sessionLiability'],'unmatchliability'=>$data[0]['unmatchliability']);
	...
	function mb_saveFancy($data){
			if($data['fancyType']==2){
					...
					$maxid = $row->maxid+1; 
					//End of Match Fancy */
					$insertData = array('HeadName'=> $data['HeadName'],'TypeID'=> $data['fancyType'],'MatchID'=> $data['mid'],'Remarks'=>$data['remarks'],'date'=>$data['date'],'time'=>$data['time'],'SessInptYes'=>$data['inputYes'],'SessInptNo'=>$data['inputNo'],'ind_fancy_selection_id'=>$data['ind_fancy_selection_id'],'SprtId'=>$data['sid'],'rateDiff'=>$data['RateDiff'],'pointDiff'=>$data['PointDiff'],'MaxStake'=>$data['MaxStake'],'NoValume'=>100,'YesValume'=>100,'active'=>1,'is_indian_fancy'=>1,'fancy_mode'=>'A');
					/*start*/
					$this->db->trans_begin();
					/*Get Match Name*/
...
//		        $remarks .= $_POST['partnership'];
	...
	{
		$this->db->select('mstruserid,usetype,mstrlock,lgnusrlckbtng,lgnusrCloseAc,IFNULL(lgnUserMaxStake,"0") as stakeLimit,lgnUserMaxProfit,lgnUserMaxLoss,IFNULL(InPlayStack,"0") as GoingInplayStakeLimit,set_timeout');
		$this->db->from('createmaster');
