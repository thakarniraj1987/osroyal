...
//return $result;
	...
	function findByMatchId($matchId=NULL){
		$this->db->select('MstCode,SportID,bet_deleted,hard_bet_deleted,seriesId');
		$this->db->from('matchmst');
		$this->db->where('MstCode',$matchId);		
		$query = $this->db->get();
		$result = $query->row_array();	
	...
	function getSettleMatchList($sportId,$matchName,$betdeleted,$fromDate,$toDate,$limit=15 , $page=1){

        $offset = ($page-1)*$limit;
        $this->db->select('SQL_CALC_FOUND_ROWS m.MstCode, m.MstDate, m.SportID, m.active, m.matchName, m.countryCode, m.marketCount, m.seriesId, m.oddsLimit, m.volumeLimit, m.HelperID,m.bet_deleted , date_format(CONVERT_TZ(m.MstDate,"+00:0","+5:30"), "%d-%m-%Y %H:%i") as createdOn ,m.hard_bet_deleted',false);
        $this->db->from('matchmst m');
        $this->db->join('market mar','m.MstCode=mar.matchId ', 'LEFT');
        $this->db->join('tblresult r','mar.Id=r.marketId', 'INNER');

        $this->db->where('mar.Name',"Match Odds");
        if($sportId!=''){
            $this->db->where('m.SportID',$sportId);
        }
        if($matchName!=''){
            $this->db->where('m.matchName like','%'.$matchName.'%');
        }

        if($fromDate!=''){
            $this->db->where("date_format(str_to_date(CONVERT_TZ(m.MstDate,\"+00:0\",\"+5:30\"), '%Y-%m-%d'), '%Y-%m-%d') >=",$fromDate );
        }
        if($toDate!=''){
            $this->db->where("date_format(str_to_date(CONVERT_TZ(m.MstDate,\"+00:0\",\"+5:30\"), '%Y-%m-%d'), '%Y-%m-%d') <=",$toDate );
        }


        if($betdeleted!=''){
            $this->db->where('m.bet_deleted ',$betdeleted);
        }
        $this->db->where('m.hard_bet_deleted ',0);
        $this->db->order_by('m.MstDate','desc');
        ...
        $query1 = $this->db->query("SELECT FOUND_ROWS() AS total;");
        $rec1=  $query1->result_array();

        return ['data'=>$rec,'total'=>$rec1[0]['total']];

    }

    function getSettleMatchListCount($sportId,$matchName,$betdeleted){

        $this->db->select('m.MstCode, m.MstDate, m.SportID, m.active, m.matchName, m.countryCode, m.marketCount, m.createdOn, m.seriesId, m.oddsLimit, m.volumeLimit, m.HelperID,m.bet_deleted');
        $this->db->from('matchmst m');
        $this->db->join('market mar','m.MstCode=mar.matchId ', 'LEFT');
        $this->db->join('tblresult r','mar.Id=r.marketId', 'INNER');
