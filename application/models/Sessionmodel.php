...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Sessionmodel extends CI_Model{		
		...
		function userScorePosition($userId,$fancyId){
            $this->db->select("*,sum(bet_value) as bet_price");
            $this->db->from("bet_entry");
            $this->db->where("FancyID",$fancyId);
            $this->db->where("UserID",$userId);
            $this->db->group_by("OddsNumber");
            $this->db->group_by("OddValue");
            $this->db->group_by("session_yes_size");
            $this->db->group_by("session_no_size");
            $this->db->order_by("OddsNumber",'asc');
            $query = $this->db->get();
            $res = $query->result_array();
