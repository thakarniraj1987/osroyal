...
/*o.get('Geteventcntr/GetSportFrmDatabase').success(function (data, status, headers, config) {
    ...
    t.RefressCaptcha = function(){
        o.get(BASE_URL+'Loginauthcontroller/loadCaptcha/' ).success(function (data, status, headers, config) {
    ...
    t.Submitcaptcha = function (captcha) {
        ...
        o({ method: 'POST', url: BASE_URL+'Loginauthcontroller/verifyCaptcha/', data: captchaData, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
	...
	t.isMarketShow=false;
         t.GetSeriesData=[];
                ...
                t.accordionLv2 = 0;
                    ...
                    o.get(BASE_URL+'Geteventcntr/getSeriesLst/' + sportsId).success(function (data, status, headers, config) {
...
t.getSeriesMatch = function (sportsId, seriesId) {
		...
		$timeout.cancel(marketTimer);
                ...
                o.get(BASE_URL+'Geteventcntr/getMatchLst/' + sportsId + '/' + seriesId).success(function (data, status, headers, config) {
                   // //
...
t.getMatchMarket = function (sportsId, matchId) {
		...
		t.accordionLv13 = matchId;
                ...
                o({ method: 'POST', url: BASE_URL+'Geteventcntr/matchMarketLstPublic/', data: marketData, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
                .success(function (data) {
                    ////
        ...
        if($state.current.name=="subadmin")
        {
            ...
            n.login(o, t, function(t) {
                ...
                else
                {
                    ...
                    //window.location.reload();
                    setTimeout(function(){document.location.href = BASE_URL+'m/?login/';},250);
           ...
           function getDynamicOdds() 
    {
       ...
       marketTimerNew = $timeout(function ()
		...
		if (t.oddsDetail != angular.isUndefinedOrNull && t.oddsDetail.length > 0 && $state.current.name=="login")
		{
			...
			o.get(BASE_URL+'GetPublicBetscntr/getUserMatchLst/' + t.SportTab).success(function (data, status, headers, config) {
                     //
                        ...
                        t.oddsDetail.find(function (item, itemIndex) {
                            ...
                            if (selectedRunner != angular.isUndefinedOrNull && selectedRunner.length > 0 && selectedRunner[0].status == "OPEN") {
                                ...
                                if (selectedRunner != angular.isUndefinedOrNull && selectedRunner[0] != null && selectedRunner[0].status == "CLOSED") {
                                    ...
                                    var selectionName1 = "";
                                    o.get(BASE_URL+'GetPublicBetscntr/getSelectionName/' + selectedRunner[0].marketId + '/' + MatchId).success(function (data, status, headers, config) {
...
getDynamicOdds();
   ...
   t.getMatchDetail = function (matchId) {
	...
	{	
        ...
        t.sportid = matchId;
	getDynamicOdds();
	var url=BASE_URL+'GetPublicBetscntr/getMatchLst/0';
 ...
 t.reset = function (MName,DateFrom,MarketId,MatchId)
{
 ...
 t.callOddsFunc = function(MarketId,MatchId) {
	...
	else
	{
        ...
        var $promise = o.get(BASE_URL+'Geteventcntr/getBackLaysOfMarket/' + MarketId + '/' + MatchId);
        var maxloop = 0;
...
t.getNameFunc = function() {
       ...
       // localStorage.clear();
        o.get(BASE_URL+'Geteventcntr/getBackLaysOfMarketSelectionName/' + t.MarketId + '/' + 0 + '/' + 0 + '/' + t.MatchId).success(function(data, status, headers, config)
            {
    ...
    t.GetMarketListId = function(MarketId,MatchId)
	{
	 ...
	 //  socket.emit('CallGetMarketListId', {auth:Bauthdata,MatchId:t.MatchId,UserId:t.UserId});
		 o({
                    method: 'GET',
                    url: BASE_URL+'Apicontroller/getMarketListing/'+t.MatchId,
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                }).success(function(data) {
                    // 
    ...
    t.BindMarket=function(result)
{
    ...
    t.getFancyList = function(marketId) {
        // stopped = $timeout(function() {
              o({
                            method: 'GET',
                            url: BASE_URL+'Apicontroller/matchLstIndianSessionPublic/'+t.MatchId+'/'+marketId,
                            headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
