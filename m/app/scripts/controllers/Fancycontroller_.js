app.controller('Fancycontroller', ['$scope', '$http', '$timeout', '$log', '$mdDialog','Dialog', function ($scope, $http, $timeout, $log, $mdDialog,Dialog)//sourabh 170103
{
    ...
    $scope.changeMatchStatus = function (matchid, status) {
        ...
        if (result) {
            $http.get( BASE_URL+'Lstsavemstrcontroller/updateFancySatatus/' + matchid + '/' + newStatus).success(function (data, status, headers, config) {
                if(data.error==0)
                {
                    var s = 0;
    ...
    $scope.changeVolumeLimit = function (limit, matchid) {
        $scope.loading = true;
        $http.get( BASE_URL+'Lstsavemstrcontroller/updateVolumeLimit/' + limit + '/' + matchid + '/').success(function (data, status, headers, config) {
    ...
    $scope.changeOddsLimit = function (limit, matchid) {
        $scope.loading = true;
        $http.get( BASE_URL+'Lstsavemstrcontroller/updateOddsLimit/' + limit + '/' + matchid + '/').success(function (data, status, headers, config) {
            if(data.error==0) {
                var s = 0;
                $http.get( BASE_URL+'Geteventcntr/getMatchLst/' + s).success(function (data, status, headers, config) {
                    $scope.match_data = data.matchLst;
