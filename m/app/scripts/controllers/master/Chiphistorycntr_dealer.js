app.controller('Chiphistorycntr', function ($scope, $http, $filter, sessionService, loginService, $rootScope, $location,$stateParams) {
    ...
    $scope.hstshow = false;
    $scope.typeDropDown = [{ value: '0', label: 'Chip' },{ value: '1', label: 'Credit Limit' },{ value: '2', label: 'Win' },{ value: '3', label: 'Lose' },{ value: '5', label: 'All' }];
    ...
    $scope.raiseevent = function (params) {        
        ...
        $http.get( BASE_URL+'Betentrycntr/Chip_history/' + params.data.UserID + '/' + ssessionService.get('type') + '/' + params.data.MatchId + '/' + params.data.MarketId + '/' + params.data.oppAcID).success(function (data, status, headers, config) {
    ...
    $scope.getChipHistoryData = function (from_date, to_date, slctType, withParent) {
        ...
        $http.get( BASE_URL+'Betentrycntr/Chip_history/' + $stateParams.UserID + '/' + from + '/' + to + '/' + type + '/' + $stateParams.usetype).success(function (data, status, headers, config) {
            $scope.chip_data = data.chips_data;
            setRowData(data.chips_data, '');
    ...
    $scope.ChipLeger = function (from_date, to_date, slctType, withParent) {
        ...
        $http.get( BASE_URL+'Betentrycntr/Chip_leger/' + $stateParams.UserID + '/' + $stateParams.usetype + '/' + drpdwn + '/' + fromDate + '/' + toDate).success(function (data, status, headers, config) {
