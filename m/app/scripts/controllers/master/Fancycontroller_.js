...
window.history.back();
	...
	$scope.fetchSessionList = function(){
		 $http.get( BASE_URL+'Apimastercontroller/get_match_session/' + stateParams.MatchId)
    ...
    $scope.changeMatchStatus = function (matchid, status) {
		...
		$http({
                    method: 'POST',
                    url: BASE_URL+'Apimastercontroller/match_status/',
                    data: {
                       match_id:matchid,
		      status:newStatus
                    },
 ...
 var s = 0;
    $http.get( BASE_URL+'Apimastercontroller/get_matches/')
            .success(function (data, status, headers, config) {
...
$scope.changeSessionStatus = function (fancyid,status) {
		...
		$http({
                    method: 'POST',
                    url: BASE_URL+'Apimastercontroller/match_session_status/',
                    data: {
                       fancy_id:fancyid,
		       status:newStatus
                    },
                ...
                var s = 0;
                $http.get( BASE_URL+'Apimastercontroller/get_match_session/' + stateParams.MatchId).success(function (data, status, headers, config) {
