...
$scope.Marketdata=JSON.parse(sessionService.get("Marketdata"))};
    ...
    $scope.GetPnL=function(){
       $scope.loading=true;
        ...
        $http.get(BASE_URL+'Betentrycntr/BetHistoryPL/' + $scope.Marketdata.UserId+'/'+$scope.Marketdata.matchId+'/'+$scope.Marketdata.MarketId+'/'+$scope.Marketdata.fancyId).success(function (data, status, headers, config) {
           $scope.loading=false;
...
console.log($scope.BetHistory);
    ...
    $scope.GetPlus_Minus_Ac=function(userId,matchId,MarketId,fancyId,mstruserid,usetype){
        ...
        $http.get(BASE_URL+'Betentrycntr/GetPlus_Minus_Ac/'+ userId+'/'+matchId+'/'+MarketId+'/'+fancyId).success(function (data, status, headers, config) {
