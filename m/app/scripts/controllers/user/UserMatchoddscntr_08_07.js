...
$scope.CommonFun=function(msg)
{
    ...
    $scope.BindIndianFancy=function(marketId)
    {
    ...
    $scope.BindSoketMarket=function(result)
    {
		...
		$scope.MarketId=result.data.marketid;
               ...
               websocket.onopen = function () {
                   console.log("Opening a connection...");
			var obj={command:'subscribe',uid:$scope.UserId};
		websocket.send(JSON.stringify(obj));
               ...
               websocket.onclose = function () {
                ...
                websocket.onerror = function (event) {
       		    $scope.CommonFun("Something went wrong.");
    		};
 ...
 console.log("print"+event.data);
                             ...
                             if(dataResult!=angular.isUndefinedOrNull)
                             {
                                 ...
                                 if(dataResult.result !=angular.isUndefinedOrNull && dataResult.result.length>0)
                                 {
				...
				else
				{
					...
					$scope.CommonFun("Records not found."); 
         ...
         //   ;
           ...
           if(inde>-1)
           {
               for(var b=0;b<$scope.FinalArray[0].runners[inde].back.length;b++)
               {
                var count = b+1;  
                try{
                    $scope.FinalArray[0].runners[inde].back[b].selected =  $scope.CallColor($scope.FinalArray[0].runners[inde].back[b].price,$scope.FinalArray[0].runners[inde].back[b].size, market[m].back[b].price,market[m].back[b].size);                 
                }
                catch(e)
                {
               ...
               try{
                $scope.FinalArray[0].runners[inde].back[b].price = market[m].back[b].price;
               }
               catch(e)
               {
               ...
               try{
                $scope.FinalArray[0].runners[inde].back[b].size = market[m].back[b].size;
                 }
                catch(e)
                {
               ...
               {
                var count = b+1;  
                $scope.FinalArray[0].runners[inde].lay[b].selected =  $scope.CallColor($scope.FinalArray[0].runners[inde].lay[b].price,$scope.FinalArray[0].runners[inde].lay[b].size, market[m].lay[b].price,market[m].lay[b].size);                 
                try{
                    $scope.FinalArray[0].runners[inde].lay[b].price = market[m].lay[b].price;
                }
                catch(e)
                {
                ...
                $scope.FinalArray[0].runners[inde].lay[b].size = market[m].lay[b].size;
               }
...
//},10000);
    ...
    $scope.UpdateAdminFancyList = function(marketId) {
      $http({
                    method: 'GET',
                    url: BASE_URL+'http://betdip.com/Apicontroller/matchLstAdminSession/'+$stateParams.MatchId+'/'+marketId,
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                }).success(function(result) {
				       ...
				       for(var i=0;i<$scope.FancyData.length;i++)
                        {
                            ...
                            {
                                var inde = $scope.AdminFancyLiveData.findIndex(img => img.ind_fancy_selection_id === 											$scope.FancyData[i].ind_fancy_selection_id);
                                if(inde > -1)
                                {
