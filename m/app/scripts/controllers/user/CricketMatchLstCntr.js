'use strict';
angular.module('ApsilonApp').controller('CricketMatchLstCntr',['$scope', '$http', 'sessionService', '$timeout', 'deviceDetector','$filter','$rootScope','$state', function ($scope, $http, sessionService, $timeout, deviceDetector,$filter,$rootScope,$state) {
    ...
    $scope.getMatchDetail = function (matchId) {
        ...
        $http.get( BASE_URL+'Geteventcntr/getUserMatchLst/' +4).success(function (data, status, headers, config) {
                        ...
                        $scope.oddsDetail.find(function (item, itemIndex) {
                            ...
                            if (selectedRunner != angular.isUndefinedOrNull && selectedRunner.length > 0 && selectedRunner[0].status == "OPEN" && selectedRunner[0].totalMatched > $scope.oddsDetail[itemIndex].totalMatched) {
                                ...
                                if (selectedRunner[0].status == "CLOSED") {
                                    ...
                                    var selectionName1 = "";
                                    $http.get( BASE_URL+'Geteventcntr/getSelectionName/' + selectedRunner[0].marketId + '/' + MatchId).success(function (data, status, headers, config) {                                                
