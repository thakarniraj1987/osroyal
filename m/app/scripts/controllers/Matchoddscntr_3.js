 ...
 var toast = $mdToast.simple().content(response.message).position('top right').hideDelay(1000);
    ...
    document.addEventListener('keydown', function(evt) {
        ...
        if(gind>-1)
        {
            ...
            $scope.FinalArray[vindx].pointDiff="";
    ...
    $scope.updateIsRupees=function(status,mid)
    {
        ...
        $http.get(BASE_URL+'Lstsavemstrcontroller/updateIsRs/' + mid + '/' + vstatus).success(function (data, status, headers, config) {
    ...
    $scope.IsManualOdds = function(items){
        ...
        $http.get(BASE_URL+'Lstsavemstrcontroller/updateMatchOddsStatus/' + items.id + '/' + $scope.IsManualVal).success(function(data, status, headers, config) {
    ...
    {
        if(!$scope.IsPointDiff)
        {
            $scope.team1layodd[indx]=$scope.team1backodd[indx]>0 ? parseFloat((parseFloat($scope.team1backodd[indx])+parseFloat(pointDiff)).toFixed(2)) : 0;
    ...
    $scope.deleteUser1 = function (betId, userId,MarketId,voids) {
        ...
        if (result) {
		  $scope.loading = true;
            $http.get(BASE_URL+'Betentrycntr/deleteGetbettingmat/' + betId + '/' + userId+'/'+MarketId+'/'+voids).success(function (data, status, headers, config) {
...
$scope.ClearAllTimeOut=function()
{
    ...
    $scope.updateManualbetAllow = function (value,item) {
        ...
        $http.get(BASE_URL+'Lstsavemstrcontroller/updateBetAllowedOnManualMatch/' + item.id + '/' + value).success(function (data, status, headers, config) {
            if (value == 0)
