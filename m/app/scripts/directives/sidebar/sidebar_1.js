    ...
    return {
       ...
       // controller: Matchoddscntr,
	    ...
	    $scope.cancelseries=null; 
            ...
            $scope.travers=function(name,children)
            {
                    ...
                    $.each(children, function(i, v) {
                        ...
                        else
                        {
                            if(v.children!=null)
                            {
                                ...
                                $scope.TraverChild(name,v.children);
                            }
                            else
                            {

                                $scope.travers(name,children[i].children);
                            }
            ...
            $scope.TraverChild=function(name,children)
            {
                if(children!=null && children.length>0)
                {
                    $.each(children, function(i, v) {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                $scope.TraverChild(name,children[i].children);
                            }
		...
		$scope.sessionusetype = sessionService.get('type');
         ...
         /*   $http.get( BASE_URL+'Geteventcntr/GetSportFrmDatabase').success(function (data, status, headers, config) {
            ...
            $scope.UpdateAccount = function (node) {
                ...
                $http({
                    method: 'POST',
                    url: BASE_URL+'Createmastercontroller/updateUserAccount/',
		...
		//
            ...
            $scope.checkUserName = function (username) {
                if (username.length < 4) {
                    ...
                    $http({
                        method: 'POST',
                        url: BASE_URL+'Createmastercontroller/CheckUserName/' + username,
                        data: username,
                        headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                    }).success(function (data) {
                        if (data.error == 0) {
