...
$scope.timerAllbets='';
    ...
    $scope.GetUserData122=function(MatchId){
...
//
		 var formData = {"is_confirm_bet":ischeckconfirm == true?'Y':'N'};
                $http({ method: 'POST', url: BASE_URL+'Apiusercontroller/confirm_bet/', data: formData, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
...
$scope.clearAll=function()
{
	...
	$scope.layArray=[];$scope.backArray=[];$scope.BackLayArray=[];$rootScope.total_liability=0;
}
					...
					if(type==0)
					{
						...
						else
						{
							...
							sum+=-recorde[j].stake;
							 ...
							 sum+=recorde[j].isback==1 ? -recorde[j].stake : recorde[j].stake;
						}
	...
	if(isSame && recorde.length>1)
					{
						...
						if($scope.prevSelection==recorde[j].selectionId && $scope.prevBack!=type)
						{
							var ind = recorde.findIndex(x=>x.unique_id==unique_id && x.isback==$scope.prevBack);
...
/////////////////////////////////////////////////////
$scope.FinalCalCulation121=function()
{
	if($rootScope.FinalTeam[2]!=angular.isUndefinedOrNull){
   $rootScope.stake2['field_'+$rootScope.stakeIds[0].UId]=$rootScope.FinalTeam[0].TeamW + $rootScope.FinalTeam[1].TeamL + $rootScope.FinalTeam[2].TeamL;
	}
	else
	{
		 $rootScope.stake2['field_'+$rootScope.stakeIds[0].UId]=$rootScope.FinalTeam[0].TeamW + $rootScope.FinalTeam[1].TeamL + 0;
	}
if($rootScope.FinalTeam[2]!=angular.isUndefinedOrNull){
   $rootScope.stake2['field_'+$rootScope.stakeIds[1].UId]=$rootScope.FinalTeam[1].TeamW + $rootScope.FinalTeam[0].TeamL+$rootScope.FinalTeam[2].TeamL;
}
else
{
 $rootScope.stake2['field_'+$rootScope.stakeIds[1].UId]=$rootScope.FinalTeam[1].TeamW + $rootScope.FinalTeam[0].TeamL+0;
}
if($rootScope.FinalTeam[2]!=angular.isUndefinedOrNull){
   $rootScope.stake2['field_'+$rootScope.stakeIds[2].UId]=$rootScope.FinalTeam[2].TeamW+$rootScope.FinalTeam[0].TeamL + $rootScope.FinalTeam[1].TeamL;
}
...
$scope.stakeval=[];
  ...
  $scope.SaveOneClick = function () {
	//	
                ...
                $http({ method: 'POST', url:BASE_URL+ 'Apiusercontroller/one_click_stake_setting/', data: stakeData, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
                .success(function (data) {
                    //
			 $scope.loading=false;
                    $scope.stakemsg = data.message;
...
$scope.Increment=function(back,type)
{

	if(type=='-')
	{
		if(back.priceVal.toFixed(2)>0)
		{
            back.priceVal= parseFloat(back.priceVal.toFixed(2))-0.01;
		}
		else
		{
            back.priceVal=0;
		}
        $scope.updateLiability(back);
	}
	else if(type=='+')
	{
        back.priceVal=parseFloat(back.priceVal)+0.01;
        $scope.updateLiability(back);
	}
}
function calltoggle()
...
$scope.$on('callAssignKeyInit',function(event,data){
	...
	$scope.Place_bet=function()
	{
	 ...
	 if(isconfirm){
		...
		if(!IsError)
		{
			var tUrl=BASE_URL+"Apiusercontroller/save_multiple_bets";
  ...
  $scope.$on('RemoveAddBackOrLay',function(event, data){
		...
		//
                ...
                $http({ method: 'POST', url: BASE_URL + 'Geteventcntr/matchMarketLst/', data: marketData, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } })
                .success(function (data) {
                    //
 ...
 $scope.updateBal=function()
    {
        ...
        $scope.upBal = $timeout(function(){   $http.get( BASE_URL+'Chipscntrl/getChipDataById/' +  sessionService.get('user_id')).success(function (data, status, headers, config) {
