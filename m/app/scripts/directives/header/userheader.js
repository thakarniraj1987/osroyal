...
$scope.getMatches = function(searchText) {
		...
		if(states !=angular.isUndefinedOrNull){
			if(states.length==0){ $scope.suggestionsContainer[0].style.height = '50px';}
...
if(series!=angular.isUndefinedOrNull){
	$scope.searchText='';
	$state.go('userDashboard.Matchodds',{'MatchId': series.matchid,'matchName':series.matchName,'date':series.MstDate,'sportId':series.SportId})
}
}
