...
*/
   ...
   $scope.getApiFrom = function(MarketId, MatchId) {
                        ...
                        //if ($scope.cValid) {
                            $http({
                                ...
                                if (data.error >= 0) {
                                        ...
                                        $scope.btnPlaceDis = false;
                                        if(isMatched==0){
                                            Dialog.show("Unmatched Bet Placed SuccessFully");
                                        }else{
                                ...
                                $http.get( BASE_URL+'Chipscntrl/getChipDataById/' + sessionService.get('slctUseID')).success(function(data, status, headers, config) {
                                   ...
                                   // alert("MaxId is"+MaxId);
                                        if(isMatched==1){
                                            $http.get( BASE_URL+'Betentrycntr/deleteGetbettingmat/' + MaxId + '/' + sessionService.get('user_id')+'/'+$stateParams.MarketId).success(function(data, status, headers, config) {
                                            ...
                                            $http.get( BASE_URL+'Betentrycntr/deleteGetbetting/' + MaxId + '/' + sessionService.get('user_id')).success(function(data, status, headers, config) {
                                                Dialog.autohide(data.message);
                                                $http.get( BASE_URL+'Betentrycntr/GatBetData/' + $stateParams.MarketId + '/' + sessionService.get('type') + '/' + sessionService.get('user_id') + '/' + $stateParams.MatchId).success(function(data, status, headers, config) {
                                                    $scope.UserData = data.betUserData;
                                                    $scope.getBetsData();
    ...
    $scope.place_bet = function() {
        ...
        $http.get( BASE_URL+'Chipscntrl/checkDeduction/' + sessionService.get('slctUseID')+"/"+$stateParams.MatchId).then(function(articles){
            ...
            if ($scope.priceVal != angular.isUndefinedOrNull) {
                $http.get( BASE_URL+'Geteventcntr/getUserMatchResult/' + sessionService.get('user_id') + '/' + sessionService.get('type')).success(function (data, status, headers, config) {
                   //////
    ...
    //code commented By Manish 30_12_2017
...
app.directive('betslist', function() { //sourabh 170118
    return {
        ...
        controller: ['$scope', '$http', '$stateParams', 'sessionService', '$interval', 'Dialog', 'get_userser', 'deviceDetector', 'speech', 'focus', '$rootScope','$timeout', function($scope, $http, $stateParams, sessionService, $interval, Dialog, get_userser, deviceDetector, speech, focus, $rootScope,$timeout) { //sourabh 170125
            ...
            //comment the interval by Manish  30_12_2017
                ...
                if (amount>=1) {
                    ...
                    get_userser.GetFancyBal($scope.FancyData[0].ID, function(response1) {
                            ...
                            if ((parseInt($scope.TotalBet) <= MaxStake - 500 && GetSumVal <= MaxStake + 500)) { //if ((GetSumVal <= MaxStake)) {
                                if ($scope.checkValidation(sessionData)) {
                                    $http({
                                        ...
                                        Dialog.autohide('Place Session Bet Successfully...');
                                        $scope.getBetsData1();
                                    });
                                }
                            } else {
                     ...
                     $scope.loadingS=false;
                        ...
                        Dialog.autohide('Min bet Value Is 1 ...');
                }
                
            };
            //var si_fancyData = $interval(callAtTimeout, 500);
    ...
    return {
        ...
        controller: ['$scope', '$http', '$stateParams', 'sessionService', '$interval', 'Dialog', 'get_userser', 'deviceDetector', 'speech', 'focus', '$rootScope','$timeout','$filter', function($scope, $http, $stateParams, sessionService, $interval, Dialog, get_userser, deviceDetector, speech, focus, $rootScope,$timeout,$filter) { //sourabh 170125
            ...
            $scope.UnMatchToMatchedData=function(Parametr){
                ////
                $http.get( BASE_URL+'Betentrycntr/updateUnMatchedData/' + Parametr.MstCode + '/' + Parametr.isBack + '/' + Parametr.MarketId + '/' + sessionService.get('type') + '/' + sessionService.get('user_id') + '/' + Parametr.MatchId).success(function(data, status, headers, config) {
                    $scope.UserData = data.betUserData;
                    $scope.getBetsData();
                });
            }
