                                                    ...
                                                    if (item.selectionId == $scope.UserData[u].SelectionId && ($scope.FinalArray[i].marketId == $scope.UserData[u].MarketId) && ($scope.UserData[u].MatchId == $stateParams.MatchId) && ($scope.UserData[u].IsMatched == 0)) {
                                                        if ($scope.UserData[u].isBack == 0) {
                                                            if (item.ex.availableToBack.length != 0 && $scope.UserData[u].Odds <= (item.ex.availableToBack[0].price + $scope.oddsLimit).toFixed(2)) {
                                                                $http.get( BASE_URL+'Betentrycntr/updateUnMatchedData/' + $scope.UserData[u].MstCode + '/' + 0 + '/' + $scope.FinalArray[i].marketId + '/' + sessionService.get('type') + '/' + sessionService.get('user_id') + '/' + $stateParams.MatchId).success(function(data, status, headers, config) {
                                                                    $scope.UserData = data.betUserData;
                                                                   // $scope.getBetsData();
                                                                });
                                                            }
                                                            ...
                                                            if (item.ex.availableToLay.length != 0 && $scope.UserData[u].Odds >= (item.ex.availableToLay[0].price + $scope.oddsLimit).toFixed(2)) {
                                                                $http.get( BASE_URL+'Betentrycntr/updateUnMatchedData/' + $scope.UserData[u].MstCode + '/' + 1 + '/' + $scope.FinalArray[i].marketId  + '/' + sessionService.get('type') + '/' + sessionService.get('user_id') + '/' + $stateParams.MatchId).success(function(data, status, headers, config) {
                                                                    $scope.UserData = data.betUserData;
                                                                   // $scope.getBetsData();
                                                                });
                                                            }
