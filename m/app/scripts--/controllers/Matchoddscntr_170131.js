app.controller('Matchoddscntr', function ($scope, $http, $stateParams, sessionService, $timeout, Dialog, $rootScope, deviceDetector, get_userser, $mdDialog) {
    ...
    $scope.getOddValue = function (item, priceVal, matchId, back_layStatus, placeName, selectionId) {
        ...
        /*get_userser.userChipSetting(function(response){
                ...
                $rootScope.userPlcBtn=response[0];
        });*/
    ...
    $scope.place_bet = function () {
        if ($scope.GetMarketBackLayData.status == "OPEN") {
            ...
            {
                if (chkValPrice > priceVal) {
                    P_and_l = (chkValPrice * stake) - stake;
                    isMatched = 0;
                    priceVal = chkValPrice;
                }
                else
    ...
    $scope.accBet = function (val) {
        switch (val) {
            ...
            case 3: $scope.acc2 = !$scope.acc2; if ($scope.acc2) $rootScope.$broadcast('getFancyList', {}); break;//sourabh 170103 new
        }
    };
    $scope.saveMatchoddsResult = function (Match_id, Sport_id, market_id, selectionId, model_result, spartName, matchName, MarketName, selectionName) {
        var marketData = { Sport_id: Sport_id, Match_id: Match_id, market_id: market_id, selectionId: selectionId, result: model_result, isFancy: 1, sportName: spartName, matchName: matchName, MarketName: MarketName, selectionName: selectionName }

        $http({ method: 'POST', url: BASE_URL+'Geteventcntr/SetResult/', data: marketData, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }).success(function (data) { $scope.message = data.status.message; });
    }
    $scope.getNameFunc = function () {

        $http.get( BASE_URL+'Geteventcntr/getBackLaysOfMarketSelectionName/' + $scope.MarketId + '/' + $scope.logInId + '/' + $scope.logInTypeId + '/' + $scope.MatchId).success(function (data, status, headers, config)///sourabh 161226 change
        {
			//
            if (data.runnerSlName[0] != angular.isUndefinedOrNull)//sourabh new 161224
                $scope.GetMarketBackLayDataSelectionName = data.runnerSlName[0].runners;
            //
            if (data.RunnerValue.length != 0)//sourabh 161228
                $scope.RunnerValue = data.RunnerValue;
    ...
    $scope.callOddsFunc = function () {
        ...
        if (matchStatus == "OPEN") {
            $http.get( BASE_URL+'Geteventcntr/getBackLaysOfMarket/' + MarketId).success(function (data, status, headers, config) {
				$http.get( BASE_URL+'Lstsavemstrcontroller/getMatchOddsLimit/' + $scope.MatchId).success(function (data, status, headers, config) {
                    if (data[0].oddsLimit != angular.isUndefinedOrNull)
						$scope.oddsLimit = parseFloat(data[0].oddsLimit);
					else
						$scope.oddsLimit = 0;
                    if (data[0].volumeLimit != angular.isUndefinedOrNull)
						$scope.volumeLimit = parseFloat(data[0].volumeLimit);
					else
						$scope.volumeLimit = 1;
                });
                if ($scope.GetMarketBackLayData == angular.isUndefinedOrNull) {
                    $scope.GetMarketBackLayData = data;
                }
                else if (MarketId == $scope.GetMarketBackLayData.marketId) {
                    $scope.GetMarketBackLayData.inplay = data.inplay;
                    $scope.GetMarketBackLayData.status = data.status;
                    matchStatus = data.status;
                    //$scope.GetMarketBackLayData.IsActive = data.IsActive;
                    $scope.GetMarketBackLayData.totalMatched = data.totalMatched;
                    var selectedRunner = data.runners;
					if (selectedRunner != angular.isUndefinedOrNull && $scope.GetMarketBackLayData.runners!= angular.isUndefinedOrNull) {
						for (var j = 0; j < $scope.GetMarketBackLayData.runners.length; j++) {
							try {
								for (var i = 0; i < selectedRunner[j].ex.availableToBack.length; i++) {
									try { $scope.GetMarketBackLayData.runners[j].ex.availableToBack[i].SELECTED = false; } catch (e) { }
									if ($scope.GetMarketBackLayData.runners[j].ex.availableToBack[i].price != angular.isUndefinedOrNull && ($scope.GetMarketBackLayData.runners[j].ex.availableToBack[i].price != selectedRunner[j].ex.availableToBack[i].price || $scope.GetMarketBackLayData.runners[j].ex.availableToBack[i].size != selectedRunner[j].ex.availableToBack[i].size)) {
										try { $scope.GetMarketBackLayData.runners[j].ex.availableToBack[i].SELECTED = true; } catch (e) { }
									}
									$scope.GetMarketBackLayData.runners[j].ex.availableToBack[i].price = selectedRunner[j].ex.availableToBack[i].price;
									$scope.GetMarketBackLayData.runners[j].ex.availableToBack[i].size = selectedRunner[j].ex.availableToBack[i].size;
								}
								for (var i = 0; i < selectedRunner[j].ex.availableToLay.length; i++) {
									try {$scope.GetMarketBackLayData.runners[j].ex.availableToLay[i].SELECTED = false; } catch (e) { }
									if ($scope.GetMarketBackLayData.runners[j].ex.availableToLay[i].price != angular.isUndefinedOrNull && ($scope.GetMarketBackLayData.runners[j].ex.availableToLay[i].price != selectedRunner[j].ex.availableToLay[i].price || $scope.GetMarketBackLayData.runners[j].ex.availableToLay[i].size != selectedRunner[j].ex.availableToLay[i].size)) {
										try { $scope.GetMarketBackLayData.runners[j].ex.availableToLay[i].SELECTED = true;} catch (e) { }
									}
									$scope.GetMarketBackLayData.runners[j].ex.availableToLay[i].price = selectedRunner[j].ex.availableToLay[i].price;
									$scope.GetMarketBackLayData.runners[j].ex.availableToLay[i].size = selectedRunner[j].ex.availableToLay[i].size;
								}
							} catch (e) {
                ...
                $scope.marketTimer = $timeout(function () {
                    if ($scope.GetMarketBackLayData.length != 0 && $scope.GetMarketBackLayData != angular.isUndefinedOrNull) {//sourabh 170107
                        for (var j = 0; j < $scope.GetMarketBackLayData.runners.length; j++) {
                            for (var i = 0; i < $scope.GetMarketBackLayData.runners[j].ex.availableToBack.length; i++) {
                                try { $scope.GetMarketBackLayData.runners[j].ex.availableToBack[i].SELECTED = false; } catch (e) { }
                            }
                            for (var i = 0; i < $scope.GetMarketBackLayData.runners[j].ex.availableToLay.length; i++) {
                                try { $scope.GetMarketBackLayData.runners[j].ex.availableToLay[i].SELECTED = false; } catch (e) { }
                            }
                        }
                        if ($scope.GetMarketBackLayData.Status != 3) {
                            if ($scope.GetMarketBackLayData.marketId != null) {
                                $scope.callOddsFunc();
                                $scope.getNameFunc();//sourabh 11-dec-2016
                            }
                        }
                    }
                    else {//sourabh 170107
                    ...
                    if (navigator.onLine) {
                        ...
                        if ($scope.netConn) {
                            $mdDialog.show({
                                ...
                                fullscreen: false,
                                controller: function DialogController(prntScope) { prntScope.netConn = false; }
                            });
                        }
                    }
                }
        ...
        else if ($scope.GetMarketBackLayData.status == "CLOSED")//sourabh new 161224
        {
            for (var j = 0; j < $scope.GetMarketBackLayData.runners.length; j++) {
                if ($scope.GetMarketBackLayData.runners[j].status == "WINNER") {
                    var selectionName1 = "";
                    if ($scope.RunnerValue != angular.isUndefinedOrNull)//sourabh 161230
                    {
                        for (var i = 0; i < $scope.RunnerValue.length; i++) {
                            if ($scope.RunnerValue[i].SelectionId == $scope.GetMarketBackLayData.runners[j].selectionId || $scope.RunnerValue[i].selectionId == $scope.GetMarketBackLayData.runners[j].selectionId)//sourabh 170102
                            {
    ...
    $scope.stakeVal = function (val, selectionId) { //sourabh 15-nov-2016
    ...
    $scope.callOddsFunc();
    $scope.countdown();
    $scope.$on("$destroy", function (event) { $timeout.cancel($scope.marketTimer); });
});
app.directive('crntusrpsn', function () {//sourabh 170118
    return {
        ...
        controller: function ($scope, $http, $stateParams, sessionService, $interval) {
            ...
            $scope.getCrntUserPosition = function () { $scope.getUserPosition($scope.crntusep_userId, $scope.crntusep_userType); }//sourabh 170127
            $scope.si_getCrntUserPosition = setInterval($scope.getCrntUserPosition, 1000);
            $scope.$on("$destroy", function (event) { clearInterval($scope.si_getCrntUserPosition); });//sourabh 170124
        }
    }
});
    ...
    return {
        ...
        controller: function ($scope, $http, $stateParams, sessionService, $interval,Dialog, get_userser, deviceDetector, speech, focus, $rootScope) {//sourabh 170125
            $scope.getFancyList = function () {//sourabh 170127
                ...
                $http({
                    method: 'POST', url: BASE_URL+'Geteventcntr/matchMatchFancy/', data: marketData,
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
                }).success(function (data) { $scope.getMatchFancy = data.getMatchFancy; $scope.getFancyUrl(); });
            }

            $scope.getFancyUrl = function (type, matchid, FancyID, sportId) {
                switch (type) {
                    case "1": return "Evenoddfancy({matchId: " + matchid + ",FancyID:" + FancyID + ",TypeID:" + type + ",matchName:'" + $stateParams.matchName + "',sportId:" + sportId + "})"; break;
                    case "2": return "Sessionfancy({matchId: " + matchid + ",FancyID:" + FancyID + ",TypeID:" + type + ",matchName:'" + $stateParams.matchName + "',sportId:" + sportId + "})"; break;
                    case "3": return "Khaddalfancy({matchId: " + matchid + ",FancyID:" + FancyID + ",TypeID:" + type + ",matchName:'" + $stateParams.matchName + "',sportId:" + sportId + "})"; break;
                    case "4": return "Lastdigit({matchId: " + matchid + ",FancyID:" + FancyID + ",TypeID:" + type + ",matchName:'" + $stateParams.matchName + "',sportId:" + sportId + "})"; break;
                    case "5": return "Updown({matchId: " + matchid + ",FancyID:" + FancyID + ",TypeID:" + type + ",matchName:'" + $stateParams.matchName + "',sportId:" + sportId + "})"; break;
                }
            }
            ...
            $scope.getMatchUnmatchData = function () {
                ...
                $http.get( BASE_URL+'Betentrycntr/GatBetData/' + $stateParams.MarketId + '/' + sessionService.get('type') + '/' + sessionService.get('user_id') + '/' + $stateParams.MatchId).success(function (data, status, headers, config) {//170131
                                ...
                                if (item.selectionId == $scope.UserData[i].SelectionId && ($scope.$parent.GetMarketBackLayData.marketId == $scope.UserData[i].MarketId) && ($scope.UserData[i].MatchId == $stateParams.MatchId) && ($scope.UserData[i].IsMatched == 0)) {
                                    ...
                                    if ($scope.UserData[i].isBack == 0) {
                                        if (item.ex.availableToBack.length != 0 && $scope.UserData[i].Odds <= (item.ex.availableToBack[0].price + $scope.$parent.oddsLimit).toFixed(2)) {
                                            $http.get( BASE_URL+'Betentrycntr/updateUnMatchedData/' + $scope.UserData[i].MstCode + '/' + 0 + '/' + $stateParams.MarketId + '/' + sessionService.get('type') + '/' + sessionService.get('user_id') + '/' + $stateParams.MatchId).success(function (data, status, headers, config) {
                                        ...
                                        {
                                            $http.get( BASE_URL+'Betentrycntr/updateUnMatchedData/' + $scope.UserData[i].MstCode + '/' + 1 + '/' + $stateParams.MarketId + '/' + sessionService.get('type') + '/' + sessionService.get('user_id') + '/' + $stateParams.MatchId).success(function (data, status, headers, config) {
            ...
            var columnDefs = [
                  ...
                  { headerName: "#", cellRenderer: function (params) { return ++params.node.id; }, cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
                  { headerName: "Runner", field: "selectionName", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
                  { headerName: "Odds", field: "Odds", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
                  { headerName: "Stake", field: "Stack", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
                  { headerName: "P&L", field: "P_L", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
                  { headerName: 'Time', field: "MstDate", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
            ];
            var columnDefsUn = [
                // this row just shows the row index, doesn't use any data from the row
                { headerName: "#", cellRenderer: function (params) { return ++params.node.id; }, cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
                ...
                { headerName: "P&L", field: "P_L", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
                { headerName: 'Time', field: "MstDate", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } },
                { headerName: 'Action', cellRenderer: ageCellRendererFunc, cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } }
            ];
            if (sessionService.get('type') == 0) {
                columnDefs.splice(1, 0, { headerName: "Client", field: "userName", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } });
                columnDefs.splice(1, 0, { headerName: "Dealer", field: "ParantName", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } });
                columnDefs.splice(1, 0, { headerName: "Master", field: "MasterName", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } });
                columnDefsUn.splice(1, 0, { headerName: "Client", field: "userName", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } });
                columnDefsUn.splice(1, 0, { headerName: "Dealer", field: "ParantName", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } });
                columnDefsUn.splice(1, 0, { headerName: "Master", field: "MasterName", cellClass: function (params) { return (params.data.isBack == 1 ? 'lay-head' : 'back-head'); } });
            }
          ...
          // return thisYear - params.data.OddValue + params.data.age;
            ...
            $scope.$on("$destroy", function (event) { clearInterval($scope.si_getMatchUnmatchData); clearInterval(si_fancyData); });//sourabh 170124
