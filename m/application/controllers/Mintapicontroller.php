...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Mintapicontroller extends CI_Controller {
		...
		//	$balance = $this->Chip_model->getLiability($userId1);
			$balance = $this->Modelcreatemaster->getUserBalance($userId1);
		...
		//	print_r($balance);die;
        	...
        	$response["data"] = $data;
        	$user_setting = $this->Modelcreatemaster->viewUserAcData($userId1);
        	if(!empty($user_setting[0])){
        		$response["user_setting"] = $user_setting[0];	
        	}
			$this->output->set_status_header(200)->set_content_type('application/json')->set_output(json_encode($response));
		}
		...
		function Save_bet(){
			...
			if ($condition[0]['resultV']==0) {
        		...
        		$this->Modelcreatemaster->updateUserBalLiablity($data['loginId']);
        		$this->output->set_status_header(200)->set_content_type('application/json')->set_output(json_encode($response));
			}else {
...
//					$data['betUserData']=$this->Betentrymodel->getBetEntry($marketId,$userTypeId,$loginId,$matchId);
		...
		function myBets($betType,$pageno,$limit){
			...
			$response = array();
			$this->load->model('Betentrymodel');
			$data = $this->Betentrymodel->myBetsPaging($userId,$betType,$pageno,$limit);
			if(count($data)==0){
				$msg = 'No record found';
			}else{
				$msg = 'My bets listing';
...
/*			$post = array(array('marketId'=>1.144844337,'selectionId'=>266950),array('marketId'=>1.144844337,'selectionId'=>266950));
		 ...
		 * @return [json]          [response]
		 */
		function matchLstIndianSession($matchId,$marketId){
			$userId = $this->globalUserId;
			$response = array();
			$this->load->model('Modeleventlst');
			$this->load->model('Modelmarket');
			...
			foreach($sessionOdds as $sessionOdd){
				$mFancyId = $sessionOdd['MFancyID'];
			
				if($sessionOdd['is_indian_fancy'] == 1 && $sessionOdd['fancy_mode'] == 'A' && !empty($getSessPrice[$mFancyId])){
		...
		function profit_lossAll(){
			$this->load->model('Betentrymodel');
			$response = array();

			$post = $_POST;
			$post['user_id'] = $this->globalUserId;
			$post['page_limit'] = DEFAULT_PAGING_LIMIT;
			
			$response["code"] = 0;
			$response["error"] = false;
    		$response["message"] = 'Profit and loss';
    		$response["data"] = $this->Betentrymodel->mbdip_profit_loss($post);
    		$this->output->set_status_header(200)->set_content_type('application/json')->set_output(json_encode($response));
		}

		function mybets_filters(){
			$this->load->model('Betentrymodel');
			$response = array();
			...
			$data = $this->Betentrymodel->mb_myBetsFilters($post);
			if(count($data)==0){
				$msg = 'No record found';
			}else{
				$msg = "My bets listing";
		...
		function BetHistoryPL($matchId,$MarketId,$fancyId){
			$this->load->model('Betentrymodel');
			$userId = $this->globalUserId;
			$response = array();
			$this->load->model('Betentrymodel');

			$data = $this->Betentrymodel->BetHistoryPL($userId,$matchId,$MarketId,$fancyId);
			if(count($data)==0){
				$msg = 'No record found';
			}else{
				$msg = "Profit bets listing";
			}

			$response["code"] = 0;
			$response["error"] = false;
        	$response["message"] = "Profit bets listing";
	        $response["data"] = $data;
			$this->output->set_status_header(200)->set_content_type('application/json')->set_output(json_encode($response));
		}
