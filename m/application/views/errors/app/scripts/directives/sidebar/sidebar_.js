    ...
    return {
        ...
        controller: function ($scope, $http, $timeout, $mdDialog, sessionService, $rootScope, get_userser, Dialog) {
            ...
            $scope.refresh_tree = function () {
                $http.get( BASE_URL+'Lstsavemstrcontroller/lstSaveMaster/' + sessionService.get('user_id') + '/' + sessionService.get('type') + '/' + sessionService.get('HelperID') + '/' + sessionService.get('Helperype'))
            ...
            function showViewSettingController($scope, $mdDialog, prntScope, node, currentScope1, Dialog) {
                ...
                $scope.updateCommission = function (oddsComm, sessionComm, otherComm, ID) {
                    $http.get( BASE_URL+'Createmastercontroller/updateCommission/' + oddsComm + '/' + sessionComm + '/' + otherComm + '/' + ID).success(function (data1, status, headers, config) {
                        Dialog.autohide(data1.message, 1000);
                        prntScope.showViewAccountForm();
                        prntScope.refresh_tree();
                    });
                ...
                $scope.ResetGloPass = function (userId) {
                    $http.get( BASE_URL+'Createmastercontroller/GetResetPasssword/')
                        .success(function (data, status, headers, config) {
                            ...
                            $http.get( BASE_URL+'Createmastercontroller/UpdateRstPasssword/' + node.id + '/' + $scope.gtPasWrd).success(function (data1, status, headers, config) {
                                if (data1.error == 0) {
                                    //alert(data1.message);
                                    $mdDialog.show({
                                        clickOutsideToClose: false,
            ...
            $scope.viewModelToggle = function (btnClicked, id) {
                ...
                $scope.modShHd = 2;
                $http.get( BASE_URL+'Createmastercontroller/viewUserAc/' + id)
                    .success(function (data, status, headers, config) {
            ...
            $scope.getMatchMarket = function (sportsId, matchId) {
                ...
                /*  $http.get( BASE_URL+'Geteventcntr/matchMarketLst').success(function (data, status, headers, config) {
            ...
            $scope.ShowHideAng = function (sportsId) {
                ...
                if ($scope.sessionusetype != 0) {
                    ...
                    //$http.get( BASE_URL+'Geteventcntr/getMatchLst/'+sportsId+'/0')
                    //.success(function (data, status, headers, config) {
                    //    $scope.GetSeriesData = data.matchLst;
                    //});
                } else {
                    $scope.GetSeriesData = angular.isUndefinedOrNull;
                    $http.get( BASE_URL+'Geteventcntr/getSeriesLst/' + sportsId)
                    .success(function (data, status, headers, config) {
