app.controller('Matchoddscntr', function ($scope, $http, $stateParams, sessionService, $timeout, Dialog, $rootScope, deviceDetector, get_userser, $mdDialog) {
    ...
    $scope.saveMatchoddsResult = function (Match_id, Sport_id, market_id, selectionId, model_result, spartName, matchName, MarketName, selectionName) {
        ...
        $http({ method: 'POST', url: 'Geteventcntr/SetResult/', data: marketData, headers: { 'Content-Type': 'application/x-www-form-urlencoded' } }).success(function (data) { $scope.message = data.status.message; });
    }
    ...
    $scope.callOddsFunc = function () {
        ...
        if (matchStatus == "OPEN") {
            $http.get('Geteventcntr/getBackLaysOfMarket/' + MarketId).success(function (data, status, headers, config) {
				$http.get('Lstsavemstrcontroller/getMatchOddsLimit/' + $scope.MatchId).success(function (data, status, headers, config) {
    ...
    return {
        ...
        controller: function ($scope, $http, $stateParams, sessionService, $interval,Dialog, get_userser, deviceDetector, speech, focus, $rootScope) {//sourabh 170125
            $scope.getFancyList = function () {//sourabh 170127
                ...
                $http({
                    method: 'POST', url: 'Geteventcntr/matchMatchFancy/', data: marketData,
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
            ...
            $scope.getMatchUnmatchData = function () {
                ...
                $http.get('Betentrycntr/GatBetData/' + $stateParams.MarketId + '/' + sessionService.get('type') + '/' + sessionService.get('user_id') + '/' + $stateParams.MatchId).success(function (data, status, headers, config) {//170131
