'use strict';
angular.module('ApsilonApp').directive('headerNotificationuser', ['$timeout', function ($timeout) {
    return {
        templateUrl: 'app/scripts/directives/header/header-notification/header-notification_user.html?var='+Math.random(),
        restrict: 'E',
        replace: true,
        scope: {},
        controller: function ($scope, $http, loginService, $mdDialog, sessionService, $rootScope, get_userser, Dialog,$state) {
            ...
            $scope.GetUsrBalance1=function(){
                $http.get('Chipscntrl/getChipDataById/' + $scope.user_id).success(function (data, status, headers, config) {
 ...
 $http.get("Loginauthcontroller/chkLoginStatus/" +  $scope.user_id).success(function (data, status, headers, config) {
		...
		if(data.status[0].lgnusrCloseAc==0 ||  data.status[0].active==0 ||  data.status[0].mstrpassword!=lgPassword || data.status[0].mstrlock==0)
		{
          ...
          //  setInterval(chkBalStatus, 10000);
          /*  $http.get('Chipscntrl/getChipDataById/' +  $scope.user_id).success(function (data, status, headers, config) {
                $scope.cipsData = data.betLibility;
