...
if(data.GetPoint.length > 1)
	...
	$scope.GetBettingTime=function()
	{
		 ...
		 $http.get('GeneralSettingsCntrl/getGeneralSettings/BET_START_TIME_BEFORE').success(function (data, status, headers, config) { 
	...
	$scope.UpdateBettingTime=function()
	{
		...
		var obj = {key:'BET_START_TIME_BEFORE',value:$scope.BetTime}
		  $http.post('GeneralSettingsCntrl/updateGeneralSettings',obj).success(function (data, status, headers, config) { 
                    Dialog.autohide(data.message);
    ...
    $scope.IsSoundCall=function(values)
    {
        debugger;
        $scope.IsSound=values;
        var audio = new Audio('http://www.flashkit.com/imagesvr_ce/flashkit/soundfx/Interfaces/Blips/Lift_Me_-Vyachesl-8596/Lift_Me_-Vyachesl-8596_hifi.mp3');
        audio.play();
        /*$scope.loading=true;
        LinkUrl ="Apiadmincontroller/save_global_market_profit";
        dataArray={"max_market_profit":values};
