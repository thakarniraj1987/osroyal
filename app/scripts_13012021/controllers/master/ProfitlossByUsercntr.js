app.controller('ProfitlossByUsercntr',['$scope', '$http', 'sessionService', '$location','$state','$stateParams', function ($scope, $http, sessionService, $location,$state,$stateParams) {
    ...
    function ageCellRendererFunc(params) {
        ...
        eSpan.addEventListener('click', function () {
            $state.go("masterDashboard.PnlPlMiSheet", { 'MarketId': params.data });          
        });
        return eSpan;
    }
   ...
   /* $scope.GetSportName=function(){
    ...
    $scope.click_test=function(sprtid){
	 $scope.loading=true;
        $http.get('Betentrycntr/profit_loss/' + $stateParams.UserID+'/'+sprtid).success(function (data, status, headers, config) {
    ...
    $scope.Allsport=function(sprtid){
        //
	 $scope.loading=true;
        $http.get('Betentrycntr/profit_lossAll/' +  $stateParams.UserID+'/'+sprtid).success(function (data, status, headers, config) {
            //
            $scope.userPL = data.userPL;
    ...
    $scope.showBet=function(pldata){
       $state.go("masterDashboard.PnlPlMiSheet", { 'MarketId': pldata });  
    };
