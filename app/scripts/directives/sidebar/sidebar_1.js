    ...
    function () {
        if (($this.val() < 1 || $this.val() > 2) && $this.val().length != 0) {
                ...
                if(uname!=angular.isUndefinedOrNull && uname!="" && uname.length>2)
                {
                    ...
                    else
                    {
                        ...
                        if(master!=angular.isUndefinedOrNull) {
                            $rootScope.tempCurrentScope1=$scope.treeNodes;
            ...
            function showAddSettingController($scope, $mdDialog, prntScope, node, sessionService, currentScope1) {
                ...
                $scope.checkDelay = function (user,type) {

                    var parentId=$scope.node.id;
                    var type=type;
                    var delay=type=='b' ? user.betDelay : user.session_delay;
                        $http({
                            method: 'GET',
                            url: 'Createmastercontroller/CheckUserDelay/' +type+'/'+parentId+'/'+delay,
                        }).success(function (data) {
            ...
            $scope.showViewSetting = function (node, currentScope1) {
                ...
                currentScope1 = currentScope1 == null ? $rootScope.tempCurrentScope1 : currentScope1;
                $scope.vcNode = node;
...
//}
			...
			var nofuser= document.getElementById('nouser').value;
                    ...
                    var userInfo = {
			            ...
			            create_no_of_child:nofuser,
                        sessionMaxStake:maxsessionStake
                    }
                ...
                $scope.ResetGloPass = function (userId) {
                    $http.get('Createmastercontroller/GetResetPasssword/')
                        .success(function (data, status, headers, config) {
                            ...
                            $http.get('Createmastercontroller/UpdateRstPasssword/' + node.id + '/' + $scope.gtPasWrd + '/' + sessionService.get('HelperID')).success(function (data1, status, headers, config) {
                                $mdDialog.show({
                                    clickOutsideToClose: false,
                                    escapeToClose: true,
           ...
           /* $http.get('Geteventcntr/GetSportFrmDatabase').success(function (data, status, headers, config) {
            ...
            $scope.UpdateViewAccount = function (useinfo, node) {
              ...
              //  try { partnership = useinfo.partnership; } catch (e) { partnership = 0; }
                ...
                var create_no_of_child=useinfo.create_no_of_child;
                ...
                var accountUserData = {
                        ...
                        sessionMaxStake:useinfo.sessionMaxStake
		...
		//
			...
			create_no_of_child:user.create_no_of_child,
                        sessionMaxStake:user.sessionStake
                    }
                }
                $http({
                    method: 'POST',
		...
		if(type==2)
		{
			for(var j=0;j<$scope.upComing.length;j++)
			{
				if(ind!=j+'u'){
		...
		else
		{
            ...
            $scope.viewModelToggle = function (btnClicked, id) {
                ...
                $scope.modShHd = 2;
                $http.get('Createmastercontroller/viewUserAc/' + id).success(function (data, status, headers, config) {
		...
		var isActive=$("li.sub-item").hasClass("active");
		var currentState=$state.current.name;
	        if(!isActive || currentState=='dashboard.Getmatchfrmapi'){
                $http.get('Geteventcntr/getMatchLst/' + sportsId + '/' + seriesId).success(function (data, status, headers, config) {
                   // //
		if(calltype == 0 || currentState=='dashboard.Getmatchfrmapi'){
			 $scope.inPlay = [];
			$scope.upComing = [];
		...
		if(sec<10)
		    sec = 0+""+sec;
 			...
 			var ind = $scope.inPlay.findIndex(x=>x.MstCode==value.MstCode);
						    if(ind==-1)
						      {
			...
			var ind = $scope.upComing.findIndex(x=>x.MstCode==value.MstCode);
			    if(ind==-1)
...
$scope.cancelSeriesTimeout = function(){
