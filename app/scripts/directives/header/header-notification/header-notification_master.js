'use strict';
angular.module('ApsilonApp').directive('headerNotificationmaster', ['$timeout', function ($timeout) {
    return {
        templateUrl: 'app/scripts/directives/header/header-notification/header-notification_master.html?var='+Math.random(),
        restrict: 'E',
        replace: true,
        scope: {},
        controller: function ($scope, $http, loginService, $mdDialog, sessionService, $rootScope, get_userser, Dialog,$state) {
            ...
            $scope.GetUsrBalance=function(){
                $http.get('Chipscntrl/getChipDataById/' + localStorage.user_id).success(function (data, status, headers, config) {
            ...
            if (sessionService.get('HelperID') == '0')
                $scope.name = localStorage.user;
            else 
                $scope.name = sessionService.get('HelperName');
            $scope.usertype = localStorage.type;
            $scope.slctUseName = localStorage.slctUseName;
            var chkBalStatus = function () { get_userser.updateUserBal(localStorage.slctUseID, function (response) {	/*console.log("service Called"+response);*/ }); 
	$scope.user_id=sessionService.get('user_id');
 $http.get("Loginauthcontroller/chkLoginStatus/" +  $scope.user_id).success(function (data, status, headers, config) {
			
		var lgPassword= sessionService.get('lgPassword');	
		if(data.status[0].lgnusrCloseAc==0 ||  data.status[0].active==0 ||  data.status[0].mstrpassword!=lgPassword || data.status[0].mstrlock==0)
		{
           ...
           $scope.$on('updateChkBal',function(event,data){
			 $scope.UpdateBalance();
		});
            $scope.upBal="";
            $scope.callbal=1;
	    ...
	    $scope.UpdateBalance();
  		var chkBals = function () { $scope.UpdateBalance()};
	   // setInterval(chkBals, 5000);
		...
		$scope.loading=true;
            ...
            function SetPasswrdCntr(scope, $mdDialog) {
                scope.SetDefaltPaswrd = function (setDftPswrd) {
                    $http.get('Createmastercontroller/UpdateCnfgPasssword/' + scope.setDftPswrd).success(function (data, status, headers, config) {
                        $mdDialog.show({
                            clickOutsideToClose: false,
                            escapeToClose: true,
