    ...
    function showViewSettingController($scope, $mdDialog, prntScope, node, Dialog,get_userser,sessionService) {
        ...
        $scope.updateCommission = function (oddsComm, sessionComm, otherComm, ID,DetectedVal) {
            ...
            if (oddsComm<=100 && sessionComm<=100 && otherComm<=100) {
                $http.get('Createmastercontroller/updateCommission/' + oddsComm + '/' + sessionComm + '/' + otherComm + '/' + ID + '/' +DetectedVal + '/' + sessionService.get('HelperID')).success(function (data1, status, headers, config) {
