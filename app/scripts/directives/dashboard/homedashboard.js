    ...
    $scope.CallCricketSocket = function (data) {
        ...
        if (dataResult != angular.isUndefinedOrNull && dataResult.length > 0) {
            ...
            for (var i = 0; i < $scope.bindCricket.length; i++) {
                ...
                var ind = $scope.socketCricket.findIndex(x => x.marketId == $scope.bindCricket[i].marketid);
                if (ind > -1) {
                    var tempResult = {};
    ...
    $scope.CallTennisSocket = function (data) {
        ...
        if (dataResult != angular.isUndefinedOrNull && dataResult.length > 0) {
            ...
            for (var i = 0; i < $scope.bindTennis.length; i++) {
                var ind = $scope.socketTennis.findIndex(x => x.marketId == $scope.bindTennis[i].marketid);
                if (ind > -1) {
                    var tempResult = {};
    ...
    $scope.CallSoccerSocket = function (data) {
        ...
        if (dataResult != angular.isUndefinedOrNull && dataResult.length > 0) {
            ...
            for (var i = 0; i < $scope.bindSoccer.length; i++) {
                var ind = $scope.socketSoccer.findIndex(x => x.marketId == $scope.bindSoccer[i].marketid);
                if (ind > -1) {
                    var tempResult = {};
    ...
    $scope.SocketMarket = function (j, newData, type) {
        ...
        if (market != angular.isUndefinedOrNull) {
            ...
            for (var m = 0; m < market.length; m++) {
                //  
                if (j > -1) {
                    var inde = $scope.FArray['f_' + type].runners.findIndex(img => img.selectionId === market[m].selectionId);
                    if (inde > -1) {
                        // $scope.FArray['f_'+type].IsMatchDisable=false;
                        //debugger
                        for (var b = 0; b < $scope.FArray['f_' + type].runners[inde].ex.availableToBack.length; b++) {
                            var count = b + 1;
                            try {
                                $scope.FArray['f_' + type].runners[inde].ex.availableToBack[b].selected = $scope.CallColor($scope.FArray['f_' + type].runners[inde].ex.availableToBack[b].price, market[m].ex.availableToBack[b].price);
                            }
                            catch (e) {

                            }
                            try {
                                $scope.FArray['f_' + type].runners[inde].ex.availableToBack[b].price = market[m].ex.availableToBack[b].price;
                            }
                            catch (e) {
                                if ($scope.FArray['f_' + type].runners[inde].ex.availableToBack[b] != angular.isUndefinedOrNull) {
                                    $scope.FArray['f_' + type].runners[inde].ex.availableToBack[b].price = "";
                                }
                            }
                            try {
                                $scope.FArray['f_' + type].runners[inde].ex.availableToBack[b].size = market[m].ex.availableToBack[b].size;
                            }
                            catch (e) {
                        ...
                        for (var b = 0; b < $scope.FArray['f_' + type].runners[inde].ex.availableToLay.length; b++) {
                            var count = b + 1;
                            try {
                                $scope.FArray['f_' + type].runners[inde].ex.availableToLay[b].selected = $scope.CallColor($scope.FArray['f_' + type].runners[inde].ex.availableToLay[b].price, market[m].ex.availableToLay[b].price);
                            }
                            catch (e) {
                            }
                            try {
                                $scope.FArray['f_' + type].runners[inde].ex.availableToLay[b].price = market[m].ex.availableToLay[b].price;
                            }
                            catch (e) {
                                if ($scope.FArray['f_' + type].runners[inde].ex.availableToLay[b] != angular.isUndefinedOrNull) {
                                    $scope.FArray['f_' + type].runners[inde].ex.availableToLay[b].price = "";
                                }
                            }
                            try {
                                $scope.FArray['f_' + type].runners[inde].ex.availableToLay[b].size = market[m].ex.availableToLay[b].size;
                            }
                            catch (e) {
                            }
                        }
