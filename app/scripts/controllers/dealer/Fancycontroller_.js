...
var authdata = Base64.encode(sessionService.get('user') + ':' +    sessionService.get('lgPassword'));
	...
	$scope.fetchSessionList = function(){
		 $http.get('Apidealercontroller/get_match_session/' + stateParams.MatchId)
...
window.history.back();
    ...
    $scope.changeMatchStatus = function (matchid, status) {
		...
		$http({
                    method: 'POST',
                    url: 'Apidealercontroller/match_status/',
                    data: {
                       match_id:matchid,
		      status:newStatus
                    },
                ...
                var s = 0;
                $http.get('Apidealercontroller/get_matches/').success(function (data, status, headers, config) {
...
$scope.changeSessionStatus = function (fancyid,status) {
		...
		$http({
                    method: 'POST',
                    url: 'Apidealercontroller/match_session_status/',
                    data: {
                       fancy_id:fancyid,
			status:newStatus
                    },
    ...
    var s = 0;
    $http.get('Apidealercontroller/get_matches/')
            .success(function (data, status, headers, config) {
                $scope.match_data = data.data;
             //   $scope.getplayer = data.getplayer;
                $scope.currentPage = 1;
