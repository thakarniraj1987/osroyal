app.controller('Profitlosscntr',['$scope', '$http', 'sessionService', '$location','$state', function ($scope, $http, sessionService, $location,$state) {
   ...
   /* $scope.GetSportName=function(){
    ...
    //$scope.GetSportName();
    if(sessionService.get('mbindProfit') != angular.isUndefinedOrNull)
    {

        $scope.FuserPL = JSON.parse(sessionService.get('mbindProfit'));
    ...
    $scope.updateProfitLoss = function(){
        ...
        {
            sessionService.set('mbindProfit',JSON.stringify($scope.userPL));
            //alert('cart updated');
            $scope.FuserPL = JSON.parse(sessionService.get('mbindProfit'));
